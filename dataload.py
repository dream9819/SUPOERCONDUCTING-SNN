...
class DataLoader:
    ...
    def load_labeled_data(self):
        ...
        self.datas = [os.path.join(DataPath, _) for _ in audio_list] 
    ...
    def spilt_train_test( self , train_rate = 0.5 , seed = 42):
        ...
        type2int,int2type,self.onehotlabel = self.onehot()
    ...
    def onehot ( self ):
        type2int = dict( ( c , i ) for i,c in enumerate( self.label_type ) )
        int2type = dict( ( i , c ) for i,c in enumerate( self.label_type ) )
        integer_encoded = [ type2int[ type ] for type in self.labels ]
        onehot_encoded = list()
        ...
        return type2int,int2type,onehot_encoded
...
class BalanceDataLoad_(DataLoader):
    ...
    def load_labeled_data(self):
        ...
        for folder_label in folders_list:
            folderpath=os.path.join(DataPath, folder_label)
    ...
    def onehot ( self ):
        ...
        type2int = dict( ( c , i ) for i,c in enumerate( self.label_type ) )
        int2type = dict( ( i , c ) for i,c in enumerate( self.label_type ) )
        test_integer_encoded = [ type2int[ type ] for type in self.test_labels ]
        train_integer_encoded = [ type2int[ type ] for type in self.train_labels ]
        onehot_test_encoded = self.onehot_encode(test_integer_encoded)
        onehot_train_encoded = self.onehot_encode(train_integer_encoded)
        return type2int,int2type,onehot_test_encoded,onehot_train_encoded
...
class NoiseLoader(DataLoader):
    ...
    def load_labeled_data(self):
        ...
        type2int,int2type,self.onehot_label = self.onehot()
        print(type2int,int2type)
